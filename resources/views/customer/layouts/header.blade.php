                ...
                <section class="mt-3 mt-md-auto search-wrapper">
                    <section class="search-box">
                        <section class="search-textbox">
                            ...
                            <form action="{{route('customer.products',[request()->category ? request()->category->id : ''])}}">
                                <input value="{{request()->search}}" name="search" id="search" type="text" class="" placeholder="جستجو ..." autocomplete="off">
                            </form>
                        </section>
                ...
                <section class="mt-3 mt-md-auto text-end">
                    @auth
                        ...
                        <section class="header-cart d-inline ps-3 border-start position-relative">
                            @auth
                            <a href="{{route('customer.sales-process.cart')}}" class="btn btn-link position-relative text-dark header-cart-link"  id="dropdownMenuButton2">
                            ...
                            @endauth
                            <section aria-labelledby="dropdownMenuButton2" class="header-cart-dropdown" data-bs-toggle="dropdown">
                                ...
                                <section class="header-cart-dropdown-body">
                                    ...
                                    @auth
                                        ...
                                        @foreach($cartItems as $item)
                                            ...
                                            @endphp
                                        <section id="section{{$item->id}}" class="header-cart-dropdown-body-item d-flex justify-content-start align-items-center">
                                            <img class="flex-shrink-1" src="{{asset($item->product->image['indexArray']['medium'])}}" alt="">
                                            <section class="w-100 text-truncate"><a class="text-decoration-none text-dark" href="{{route('customer.market.product',[$item->product->slug])}}">{{$item->product->name}}</a></section>
                                            <section class="flex-shrink-1"><a class="text-muted text-decoration-none p-1" data-url="{{route('customer.sales-process.remove-from-cart',[$item->id])}}" id="delete{{$item->id}}" onclick="deleteRecord({{$item->id}})" ><i class="fa fa-trash-alt"></i></a></section>
                                        </section>
                                        @endforeach
                                    @endauth
                                ...
                                <section class="header-cart-dropdown-footer border-top d-flex justify-content-between align-items-center p-2">
                                    @auth
                                    <section class=""><section>مبلغ قابل پرداخت</section><section> {{\App\Helpers\priceFormat($totalProductPrice - $totalDiscount)}} تومان</section></section>
                                    @endauth
