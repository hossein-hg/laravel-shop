                    ...
                    <table class="table table-striped table-hover">
                        ...
                        @foreach($emails as $key =>$email)
                            <tr>
                                ...
                                <td>
                                    <label for="">
                                        <input id="{{$email->id}}" type="checkbox" @if($email->status == 1) checked @endif onchange="changeStatus({{$email->id}})" data-url="{{route('admin.notify.email.status',[$email->id])}}">
                                    </label>
                                </td>
                                <td class="width-16-rem text-left d-flex">
                                    ...
                                    <form method="post" action="{{route('admin.notify.email.destroy',[$email->id])}}">
                                        @csrf
                                        @method('delete')
