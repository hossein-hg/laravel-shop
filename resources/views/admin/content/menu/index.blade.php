                ...
                <table class="table table-striped table-hover">
                    ...
                    @foreach($menus as $key => $menu)
                        <tr>
                            <th>{{$key+1}}</th>
                            <td>{{$menu->name}}</td>
                            <td>{{!empty($menu->parent) ? $menu->parent->name : 'منو اصلی'}}</td>
                            <td>{{$menu->url}}</td>
                            <td>
                                <label for="">
                                    <input id="status-{{$menu->id}}" type="checkbox" @if($menu->status == 1) checked @endif onchange="changeStatus({{$menu->id}})" data-url="{{route('admin.content.menu.status',[$menu->id])}}">
                                </label>
                            </td>
