                ...
                <table class="table table-striped table-hover">
                    ...
                    @foreach($admins as $key => $admin)
                        <tr>
                            ...
                            <td><img src="{{asset($admin->profile_photo_path)}}" alt="" width="70" height="60">	</td>
                            <td>
                                <label for="">
                                    <input id="{{$admin->id}}" type="checkbox" @if($admin->activation == 1) checked @endif onchange="changeActivation({{$admin->id}})" data-url="{{route('admin.user.admin-user.activation',[$admin->id])}}">
                                </label>
                            </td>
                            <td>
                                <label for="">
                                    <input id="status-{{$admin->id}}" type="checkbox" @if($admin->status == 1) checked @endif onchange="changeStatus({{$admin->id}})" data-url1="{{route('admin.user.admin-user.status',[$admin->id])}}">
                                </label>
                            </td>
                            <td>سوپر ادمین	</td>
                ...
                success: function (response){
                    if (response.status)
                    {
                        if (response.checked)
                        {
                            ...
                            successToast('کاربر ادمین با موفقیت غیرفعال شد')
                        }
                    }
                    else {
            ...
            $.ajax({
                ...
                success: function (response){
                    if (response.status)
                    {
                        ...
                        {
                            element.prop('checked',true)
                            successToast('وضعیت کاربر ادمین با موفقیت فعال شد')
                        }
                        else {
                            element.prop('checked',false)
                            successToast('وضعیت کاربر ادمین با موفقیت غیرفعال شد')
                        }
                    }
                    else {
                        element.prop('checked',elementValue)
