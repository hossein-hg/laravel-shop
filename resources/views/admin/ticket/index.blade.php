                ...
                <table class="table table-striped table-hover">
                    ...
                    @foreach($tickets as $key => $ticket)
                        <tr>
                            ...
                            <td>{{ $ticket->parent ? $ticket->parent->subject : ''  }}</td>
                            <td class="width-16-rem text-left">
                                <a href="{{route('admin.ticket.show',[$ticket->id])}}" class="btn btn-info btn-sm"><i class="fa fa-eye"></i> مشاهده</a>
                                <a href="{{route('admin.ticket.change',[$ticket->id])}}" class="btn {{$ticket->status ? 'btn-warning' : "btn-danger"}} btn-sm"><i class="fa fa-check"></i> {{$ticket->status ? 'باز کردن' : "بستن"}}</a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
