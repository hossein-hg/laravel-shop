                ...
                <table class="table table-striped table-hover">
                    ...
                    @foreach($comments as $key => $comment)
                        <tr>
                            ...
                            <td>
                                <label for="">
                                    <input id="status-{{$comment->id}}" type="checkbox" @if($comment->status == 1) checked @endif onchange="changeStatus({{$comment->id}})" data-url="{{route('admin.market.comment.status',[$comment->id])}}">
                                </label>
                            </td>
                            <td class="width-16-rem text-left">
                                <a href="{{ route('admin.market.comment.show',[$comment->id]) }}" class="btn btn-info btn-sm"><i class="fa fa-eye"></i> نمایش</a>
                                <a id="approved-{{$comment->id}}" class="btn @if($comment->approved == 1) btn-danger @else btn-success @endif btn-sm " href=""  onclick="changeApproved(event,{{$comment->id}})" data-url1="{{route('admin.market.comment.approved',[$comment->id])}}"><i class="fa fa-check"></i> {{$comment->approved == 1 ? 'عدم تایید' : 'تایید'}}</a>
                            </td>
                        </tr>
                    @endforeach
