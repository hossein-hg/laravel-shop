                    ...
                    <table class="table table-striped table-hover">
                        ...
                        @foreach($brands as $key => $brand)
                            <tr id="tr{{$brand->id}}">
                                ...
                                <td><img src="{{asset($brand->logo['indexArray']['small'])}}" class="max-height-2rem" alt=""> </td>
                                <td><label for="">
                                        <input data-url="{{route('admin.market.brand.status',[$brand->id])}}" id="{{$brand->id}}" type="checkbox" {{$brand->status == 1 ? 'checked' : ''}} onchange="changeStatus({{$brand->id}})">
                                    </label> </td>
                                <td class="width-16-rem text-left">
                                    <a href="{{route('admin.market.brand.edit',[$brand->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> ویرایش</a>
                                    <a  id="delete{{$brand->id}}" onclick="deleteRecord({{$brand->id}})" data-url="{{route('admin.market.brand.destroy',[$brand->id])}}" class="btn btn-danger btn-sm text-white"><i class="fa fa-edit"></i> حذف</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
