                ...
                <table class="table table-striped table-hover h-150px">
                    ...
                    @foreach($products as $key => $product)
                        <tr>
                            <th>{{$key+1}}</th>
                            <td>{{$product->name}}</td>
                            <td><img width="50" height="50" src="{{asset($product->image['indexArray'][$product->image['currentImage']])}}" alt=""></td>
                            <td>{{$product->price}} تومان</td>

                            <td>{{$product->category ? $product->category->name : ''}}</td>
                            <td>{{$product->brand ? $product->brand->persian_name : ''}}</td>
                            <td>{{jalaliDate($product->published_at,'H:i:s Y-m-d')}}	</td>
                            <td class="width-8-rem text-left">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                        ...
                                        <a href="{{route('admin.market.product-color.index',[$product->id])}}" class="dropdown-item text-right"><i class="fa fa-list-ul"></i> مدیریت رنگ ها </a>
