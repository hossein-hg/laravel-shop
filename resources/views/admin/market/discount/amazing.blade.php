                ...
                <table class="table table-striped table-hover">
                    ...
                    @foreach($amazingSales as $key => $amazingSale)
                        <tr>
                            ...
                            <td>
                                <label for="">
                                    <input id="{{$amazingSale->id}}" type="checkbox" @if($amazingSale->status == 1) checked @endif onchange="changeStatus({{$amazingSale->id}})" data-url="{{route('admin.market.discount.amazingSale.status',[$amazingSale->id])}}">
                                </label>
                            </td>
                            ...
                            <td class="width-16-rem text-left d-flex">
                                ...
                                <form method="post" action="{{route('admin.market.discount.amazingSale.destroy',[$amazingSale->id])}}">
                                    @csrf
                                    @method('delete')
                ...
                success: function (response){
                    if (response.status)
                    {
                        if (response.checked)
                        {
                            ...
                            successToast('فروش شگفت انگیز با موفقیت غیرفعال شد')
                        }
                    }
                    else {
