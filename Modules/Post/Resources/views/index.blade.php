                ...
                <section class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tr id="tr{{$post->id}}">
                            ...
                            <td><img src="{{asset($post->image['indexArray']['small'])}}" class="max-height-2rem" alt=""> </td>
                            <td>
                                {{\App\Helpers\jalaliDate($post->published_at)}}
                            </td>
                            <td>
                                <input id="{{$post->id}}" data-url="{{route('admin.content.post.status',[$post->id])}}" onchange="changeStatus({{$post->id}})" type="checkbox" {{$post->status == 1 ? 'checked' : ''}}>
                            </td>
                            <td class="width-16-rem text-left">
                                <a href="{{route('admin.content.post.edit',[$post->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> ویرایش</a>
                                <a id="delete{{$post->id}}" onclick="deletePost({{$post->id}})"  class="btn btn-danger btn-sm text-white" data-url="{{route('admin.content.post.destroy',[$post->id])}}"><i class="fa fa-trash-alt"></i> حذف</a>
                            </td>
                        </tr>
                        @endforeach
                        </tbody>
                    ...
                    $.ajax({
                        ...
                        success:(res)=>{
                            if (res.status){
                                let trTag = $('#tr'+id)
                                trTag.remove()
                                successAlert('پست با موفقیت حذف شد')
                            }
                            else {
